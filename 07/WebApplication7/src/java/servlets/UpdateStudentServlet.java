...
@WebServlet(name = "UpdateStudentServlet", urlPatterns = {"/submitstudentupdating.jsp"})
public class UpdateStudentServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            int studentId = validateStudentId((String) session.getAttribute("studentId"));
            String studentName = validateStudentName((String) session.getAttribute("studentName"));
            int groupId = validateGroupId((String) session.getAttribute("groupId"));
            controller.updateStudent(studentId, studentName, groupId);
            RequestDispatcher dispatcher = request.getRequestDispatcher("/updatestudentresult.jsp");
            if (dispatcher != null) {
                dispatcher.forward(request, response);
            }
        } catch (DataUpdatingException | EmptyStringException |
                NonexistentEntityException | SQLException | StringFormatException ex) {
            printException(ex);
